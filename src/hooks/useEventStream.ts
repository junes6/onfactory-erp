...
export type StreamEventKind = 'notification' | 'proposal' | 'work' | 'message' | 'activity' | 'calendar' | 'wiki' | 'resync'
...
export function useEventStream(enabled: boolean, onEvent: (event: StreamEvent) => void) {
  ...
  useEffect(() => {
    ...
    const kinds: StreamEventKind[] = ['notification', 'proposal', 'work', 'message', 'activity', 'calendar', 'wiki', 'resync']
    const open = () => {
      if (closed) return
