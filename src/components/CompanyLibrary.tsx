...
import { Bot, CheckCircle2, Cloud, Database, Download, FileArchive, FileText, FolderSearch, HardDrive, LockKeyhole, Pencil, Plus, Search, Server, Trash2, Upload, Users, X } from 'lucide-react'
...
const initialNas: NasSettings = { provider: 'webdav', endpoint: '', share: '', basePath: '/온팩토리', account: '', status: '설정 필요' }
  ...
  return <div className="library-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section ref={dialogRef} className="library-modal" role="dialog" aria-modal="true" aria-labelledby="library-modal-title">
      ...
      <form onSubmit={async (event: FormEvent<HTMLFormElement>) => {
        ...
        {!document && <section className="library-dropzone"><input ref={fileRef} className="sr-only" type="file" onChange={(event) => { const selected = event.target.files?.[0] ?? null; setFile(selected); if (selected) { const nameInput = event.currentTarget.form?.elements.namedItem('name') as HTMLInputElement | null; if (nameInput && !nameInput.value) nameInput.value = selected.name } }} /><Upload size={26} /><div><strong>{file?.name ?? '파일을 선택해 주세요'}</strong><span>{file ? humanSize(file.size) : 'PDF, 문서, 이미지, 압축파일 · 최대 10MB'}</span></div><button className="button secondary" type="button" onClick={() => fileRef.current?.click()}>파일 선택</button></section>}
        <div className="library-form-grid">
          ...
          <label><span>분류</span><select name="category" defaultValue={document?.category ?? '공통자료'}><option>공통자료</option><option>식품안전·인증</option><option>제품·표시사항</option><option>생산·품질</option><option>재고·물류</option><option>판매·계약</option><option>인사·교육</option><option>회의·업무일지</option></select></label>
        ...
        <footer><button className="button ghost" type="button" onClick={onClose}>취소</button><button className="button primary" type="submit" disabled={busy}><CheckCircle2 size={18} /> {busy ? '저장 중…' : document ? '변경사항 저장' : '안전하게 업로드'}</button></footer>
      </form>
    </section>
  </div>
}
...
export function CompanyLibrary({ workspaceScope, canManage, currentUserId, companyName, industryType = 'food_manufacturing', onToast }: { workspaceScope?: string; canManage: boolean; currentUserId: string; companyName: string; industryType?: string; onToast: (message: string) => void }) {
  ...
  const askAi = async (event: FormEvent) => {
    ...
    try { const response = await fetch('/api/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ feature: 'document-search', messages: [{ role: 'user', content: `기업 자료실에서 다음 요청에 맞는 자료를 찾아 주세요: ${aiQuery.trim()}` }], context: { company: companyName, accessibleDocuments: documents.map(({ id, name, category: documentCategory, tags, summary, uploadedAt }) => ({ id, name, category: documentCategory, tags, summary, uploadedAt })) } }) }); const body = await response.json() as { text?: string; error?: { message?: string } }; if (!response.ok || !body.text) throw new Error(body.error?.message || 'AI 검색에 실패했습니다.'); setAiAnswer(body.text) } catch (error) { setAiAnswer(error instanceof Error ? error.message : 'AI 검색에 실패했습니다.') } finally { setAiBusy(false) }
  }
  return <div className="library-page"><header className="library-page-head"><div><span>COMPANY KNOWLEDGE</span><h1>기업 자료실</h1><p>권한에 맞는 회사 자료를 안전하게 보관하고, AI에게 필요한 문서를 바로 찾도록 요청하세요.</p></div><div>{canManage && <button className="button secondary" type="button" onClick={() => setNasOpen(true)}><Database size={18} /> NAS 설정</button>}<button className="button primary" type="button" onClick={() => setEditing('new')}><Upload size={18} /> 자료 업로드</button></div></header>
    <section className="library-ai-search"><span><Bot size={24} /></span><form onSubmit={askAi}><label htmlFor="library-ai-query">AI 자료 찾기</label><div><input id="library-ai-query" value={aiQuery} onChange={(event) => setAiQuery(event.target.value)} placeholder={industryType === 'it_services' ? '예: 최근 프로젝트 계약서 최종본 찾아줘' : '예: 최근 자가품질검사 성적서 찾아줘'} /><button type="submit" disabled={aiBusy || !aiQuery.trim()}>{aiBusy ? '찾는 중…' : 'AI에게 찾기'}</button></div></form>{aiAnswer && <div className="library-ai-answer"><strong>검색 결과</strong><p>{aiAnswer}</p><button type="button" aria-label="검색 결과 닫기" onClick={() => setAiAnswer('')}><X size={16} /></button></div>}</section>
    <section className="library-storage-strip"><div><HardDrive size={19} /><span>온팩토리 보관함</span><strong>{documents.filter((item) => item.storage === 'local').length}개</strong></div><div><Cloud size={19} /><span>NAS 연결</span><strong>{nas.status}</strong></div><div><Users size={19} /><span>내 열람 가능</span><strong>{documents.length}개</strong></div><div><FileArchive size={19} /><span>총 용량</span><strong>{humanSize(documents.reduce((sum, item) => sum + item.size, 0))}</strong></div></section>
