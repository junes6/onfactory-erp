...
import { Building2, Check, Download, ExternalLink, FileArchive, FileCheck2, FolderOpen, Landmark, Paperclip, Pencil, Trash2, Upload, X } from 'lucide-react'
import { useWorkspaceState } from '../hooks/useWorkspaceState'
import { formatDateLabel, seoulDateInputValue, toIsoUtc } from '../utils/dateTime'
import {
...
type EvidenceBucket = '매출' | '매입' | '급여' | '경비' | '신고·납부' | '기타'
...
export function TaxWorkspace({ workspaceScope, canManage, currentUserId, currentUserName, onToast }: { workspaceScope?: string; canManage: boolean; currentUserId: string; currentUserName: string; onToast: (message: string) => void }) {
  const currentYear = Number(seoulDateInputValue().slice(0, 4))
  ...
  const provided = useMemo(() => profile ? buildProvidedTaxSchedule(profile, year) : [], [profile, year])
  ...
  const yearDocuments = documents.filter((document) => document.tags.includes('tax-evidence') && document.tags.includes(`tax-year:${year}`))
  ...
  useEffect(() => { if (workspaceScope) void loadDocuments() }, [workspaceScope]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const uploadEvidence = async (files: File[]) => {
    ...
    try {
      await uploadDocumentAttachments(files, {
        workspaceScope,
        category: '세무·회계',
        summary: `${year}년 ${uploadBucket} 세무 증빙`,
        tags: ['tax-evidence', `tax-year:${year}`, `tax-bucket:${uploadBucket}`],
      })
      await loadDocuments()
      onToast(`${year}년 ${uploadBucket} 증빙 ${files.length}개를 보관했습니다.`)
  ...
  const exportEvidence = async () => {
    setExporting(true)
    try {
      const response = await fetch(`/api/tax/evidence-export?year=${year}`, { headers: workspaceHeaders(workspaceScope) })
      if (!response.ok) throw new Error(await responseMessage(response, '세무사 전달 묶음을 만들지 못했습니다.'))
      const blob = await response.blob()
      const url = URL.createObjectURL(blob)
      try {
        const anchor = document.createElement('a')
        anchor.href = url; anchor.download = `${year}_세무사_전달자료.zip`; document.body.appendChild(anchor); anchor.click(); anchor.remove()
      } finally { window.setTimeout(() => URL.revokeObjectURL(url), 0) }
      onToast(`${year}년 증빙 ${response.headers.get('x-tax-evidence-files') ?? yearDocuments.length}개를 묶었습니다.`)
  ...
  const pendingCount = provided.filter((schedule) => statusFor(schedule) === '예정').length
  const completeCount = provided.filter((schedule) => statusFor(schedule) === '납부 완료').length
  const evidencePanel = <section className="tax-section-panel" aria-labelledby="tax-evidence-title">
    <header className="tax-section-head"><div><FileArchive size={18} /><div><h2 id="tax-evidence-title">{year}년 증빙 파일함</h2><span>연간 자료를 세무사에게 한 번에 전달</span></div></div>{canManage && <button className="button secondary" type="button" aria-label="세무사 전달 묶음 받기" disabled={exporting || yearDocuments.length === 0} onClick={() => void exportEvidence()}><Download size={16} /> {exporting ? '묶는 중…' : '전달 ZIP 받기'}</button>}</header>
    ...
    <div className="tax-evidence-list" aria-busy={documentsLoading || uploading}>{EVIDENCE_BUCKETS.map((bucket) => { const files = yearDocuments.filter((document) => evidenceBucket(document) === bucket.id); return <article className="tax-evidence-row" key={bucket.id}>
      ...
      <button className="button ghost tax-upload-button" type="button" aria-label={`${bucket.id} 증빙 파일 넣기`} disabled={uploading} onClick={() => { setUploadBucket(bucket.id); fileInputRef.current?.click() }}><Upload size={15} /> 추가</button>
    </article> })}</div>
    {!documentsLoading && yearDocuments.length === 0 && <p className="tax-evidence-empty">아직 {year}년 증빙이 없습니다. 매출·매입·급여 등 알맞은 칸에 파일을 넣으면 연도와 분류가 자동 저장됩니다.</p>}
  </section>
  return <div className="tax-workspace">
    <section className="tax-workspace-toolbar" aria-label="세무 연도와 회사 설정">
      <div><strong>우리 회사 세무 캘린더</strong><span>회사 조건에 맞는 국세청 기준 일정을 자동으로 정리합니다.</span></div>
      <div className="tax-toolbar-actions">
        ...
        {canManage && profile && <button className="button secondary" type="button" onClick={() => setProfileOpen(true)}><Pencil size={16} /> 회사 조건 수정</button>}
      </div>
    </section>

    {!profile ? <><section className="tax-profile-callout">
      <span><Building2 size={24} /></span><div><h2>회사 세무 조건을 한 번 설정해 주세요</h2><p>법인·개인, 부가세 유형, 결산월과 급여 지급 여부만 고르면 사용자가 일정을 일일이 등록하지 않아도 해당 연도 일정을 구성합니다.</p></div>
      {canManage ? <button className="button primary" type="button" onClick={() => setProfileOpen(true)}>회사 조건 설정</button> : <span>회사 관리자 설정 대기</span>}
    </section><div className="tax-evidence-standalone">{evidencePanel}</div></> : <>
      <section className="tax-progress-strip" aria-label={`${year}년 세무 준비 현황`}>
        <div><Landmark size={18} /><span>확인할 일정</span><strong>{pendingCount}건</strong></div>
      ...
      <div className="tax-workspace-grid">
        <section className="tax-section-panel" aria-labelledby="tax-schedule-title">
          <header className="tax-section-head"><div><Landmark size={18} /><div><h2 id="tax-schedule-title">{year}년 챙길 세무 일정</h2><span>국세청 공식 안내를 회사 조건에 맞춰 적용</span></div></div><small>기준 확인 {formatDateLabel(provided[0]?.checkedAt, true, false)}</small></header>
          <div className="tax-schedule-list">{provided.map((schedule) => { const status = statusFor(schedule); return <article className="tax-schedule-row" key={schedule.id}>
            ...
            <div className="tax-schedule-main"><div><StatusBadge dot tone={taxStatusTone(status, schedule.dueDate, today)}>{taxStatusLabel(status, schedule.dueDate, today)}</StatusBadge><strong>{schedule.title}</strong></div><p>{schedule.description}</p><small>{schedule.appliesTo} · <a href={schedule.sourceUrl} target="_blank" rel="noreferrer">{schedule.sourceLabel} <ExternalLink size={11} /></a></small></div>
    ...
    {legacyRecords.length > 0 && <details className="tax-legacy"><summary>이전에 직접 등록한 회사 일정 {legacyRecords.length}건</summary><div>{legacyRecords.map((record) => <span key={record.id}><strong>{record.title}</strong><small>{record.dueDate ? formatDateLabel(record.dueDate) : '기한 미정'} · {record.status}</small></span>)}</div></details>}
    {profileOpen && <TaxProfileEditor record={profileRecord} currentUserName={currentUserName} onClose={() => setProfileOpen(false)} onSave={async (next) => {
...
function TaxProfileEditor({ record, currentUserName, onClose, onSave }: { record?: TaxRecord; currentUserName: string; onClose: () => void; onSave: (next: TaxRecord) => Promise<boolean> }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal-card it-modal tax-profile-modal" role="dialog" aria-modal="true" aria-labelledby="tax-profile-title">
    <header><div><span className="eyebrow">TAX PROFILE</span><h2 id="tax-profile-title">회사 세무 조건</h2><p>일정 적용에 필요한 조건만 저장합니다. 실제 신고 전에는 담당 세무사와 적용 여부를 확인해 주세요.</p></div><button className="icon-button" type="button" aria-label="닫기" onClick={onClose}><X size={21} /></button></header>
    <form onSubmit={submit}><div className="form-grid"><label className="form-field"><span>사업자 유형</span><select name="entityType" defaultValue={record?.entityType ?? 'corporation'}><option value="corporation">법인사업자</option><option value="individual">개인사업자</option></select></label><label className="form-field"><span>부가가치세 유형</span><select name="vatType" defaultValue={record?.vatType ?? 'general'}><option value="general">일반과세</option><option value="simplified">간이과세</option><option value="exempt">면세</option></select></label></div>
      <div className="form-grid"><label className="form-field"><span>결산월</span><select name="fiscalYearEndMonth" defaultValue={record?.fiscalYearEndMonth ?? 12}>{Array.from({ length: 12 }, (_, index) => index + 1).map((month) => <option key={month} value={month}>{month}월</option>)}</select></label><label className="form-field"><span>급여·원천징수 여부</span><select name="hasPayroll" defaultValue={record?.hasPayroll ? 'yes' : 'no'}><option value="yes">있음</option><option value="no">없음</option></select></label></div>
      <label className="form-field full"><span>원천세 납부 주기</span><select name="withholdingCycle" defaultValue={record?.withholdingCycle ?? 'monthly'}><option value="monthly">매월 납부</option><option value="semiannual">반기납부 승인받음</option></select></label>
